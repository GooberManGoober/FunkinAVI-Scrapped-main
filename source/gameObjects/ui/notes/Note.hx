...
class Note extends FlxSprite
{
	...
	public function new(strumTime:Float, noteData:Int, ?prevNote:Note, ?sustainNote:Bool = false, ?inEditor:Bool = false)
	{
		...
		{
			alpha = ClientPrefs.arrowAlpha;
			multAlpha = ClientPrefs.arrowAlpha;
		}

		if (isSustainNote && prevNote != null)
		{
			alpha = ClientPrefs.holdAlpha;
			multAlpha = ClientPrefs.holdAlpha;
	...
	function reloadNote(?prefix:String = '', ?texture:String = '', ?suffix:String = '') {
		...
		if(texture.length < 1) {
			...
			if(skin == null || skin.length < 1) {
				switch (PlayState.SONG.song)
				{
					case "Isolated" | "Devilish Deal" | "Lunacy" | "Delusional" | "Hunted" | "Twisted Grins" | "Laugh Track" | "Cycled Sins":
						skin = "GreyNOTE_assets";
