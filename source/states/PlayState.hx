...
class PlayState extends MusicBeatState
{
	...
	public static var bloomEffect:FlxRuntimeShader = new FlxRuntimeShader(Shaders.bloom, null, 120);
	...
	override public function create()
	{
		...
		{	
			case 'menuSongs' | 'abandonedStreet' | 'alleyway' | 'ddStage':
				...
				if (ClientPrefs.downScroll || curStage == "menuSongs")
				{
			...
			default:
				...
				spectraSongTime.cameras = [camHUD];
		...
		{	
			case 'menuSongs' | 'abandonedStreet' | 'alleyway' | 'ddStage':
				scoreTxt = new FlxText(0, ((curStage == "menuSongs" || curStage == "waltRoom") ? (ClientPrefs.downScroll ? 15 : 675) : healthBarBG.y + 36), FlxG.width, "", 28);
		...
		if (curStage == "waltRoom")
		{
			...
			for (funny in [spaceBarCounter, mercyBoostIcon])
		...
		windowTimer = new FlxTimer().start(5, function(tmr:FlxTimer)
		{
			if (GameData.episode1FPLock != "unlocked")
				windowName = "Funkin.avi: Scrapped - Episode 1 - Isolated [NORMAL]"
			else
				windowName = "Funkin.avi: Scrapped - " + 
				(isStoryMode ? curEpisode + " - " : "Freeplay - ") + 
				(SONG.song == "Dont Cross" ? "Don't Cross!" : SONG.song) + 
			...
			switch (SONG.song)
			{
				...
				case 'Hunted Legacy':
					...
					if (!ClientPrefs.lowQuality)
					{
						...
						@:privateAccess for(_camHUD in [camHUD]) _camHUD.setFilters([
							new ShaderFilter(grayScale),
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					/*else
					{
						...
						{
							case "Delusional":
								for (i in [camHUD, camGame, camBars])
									if (i.visible)
										i.visible = false;
	...
	override function beatHit()
	{
		...
		switch (SONG.song)
		{
			...
			case 'Delusional':
				...
				switch (curBeat)
				{
					...
					case 480:
						...
						#end
						#end
						windowName = "Funkin.avi: Scrapped - " + (isStoryMode ? curEpisode + " - " : "Freeplay - ") + "Regret [________]";
						lime.app.Application.current.window.title = windowName;
						// no healthbar to add more onto the atmosphere of this section
						camGame.visible = true;
						camHUD.visible = true;
					case 484:
						windowName = "Funkin.avi: Scrapped - " + (isStoryMode ? curEpisode + " - " : "Freeplay - ") + "Regret [P_______]";
						lime.app.Application.current.window.title = windowName;
					case 488:
						windowName = "Funkin.avi: Scrapped - " + (isStoryMode ? curEpisode + " - " : "Freeplay - ") + "Regret [PE______]";
						lime.app.Application.current.window.title = windowName;
					case 492:
						windowName = "Funkin.avi: Scrapped - " + (isStoryMode ? curEpisode + " - " : "Freeplay - ") + "Regret [PEA_____]";
						lime.app.Application.current.window.title = windowName;
					case 496:
						windowName = "Funkin.avi: Scrapped - " + (isStoryMode ? curEpisode + " - " : "Freeplay - ") + "Regret [PEAC____]";
						lime.app.Application.current.window.title = windowName;
					case 500:
						windowName = "Funkin.avi: Scrapped - " + (isStoryMode ? curEpisode + " - " : "Freeplay - ") + "Regret [PEACE___]";
						lime.app.Application.current.window.title = windowName;
					case 504:
						windowName = "Funkin.avi: Scrapped - " + (isStoryMode ? curEpisode + " - " : "Freeplay - ") + "Regret [PEACEF__]";
						lime.app.Application.current.window.title = windowName;
					case 508:
						windowName = "Funkin.avi: Scrapped - " + (isStoryMode ? curEpisode + " - " : "Freeplay - ") + "Regret [PEACEFU_]";
						lime.app.Application.current.window.title = windowName;
						FlxTween.tween(boyfriend, {alpha: 0.45}, 2.5, {ease: FlxEase.expoOut});
					case 512:
						windowName = "Funkin.avi: Scrapped - " + (isStoryMode ? curEpisode + " - " : "Freeplay - ") + "Regret [PEACEFUL]";
						lime.app.Application.current.window.title = windowName;
					case 672:
					...
					case 744:
						...
						#end
						#end
						windowName = "Funkin.avi: Scrapped - " + (isStoryMode ? curEpisode + " - " : "Freeplay - ") + SONG.song + " [" + FreeplayState.getDiffRank() + "]";
						lime.app.Application.current.window.title = windowName;
						camVideo.visible = false;
						camGame.alpha = 1;
						camHUD.visible = true;
			...
			case "Bless":
				switch (curBeat)
				{
					...
					case 212:
						...
						for (textShit in [songTxt, watermarkTxt, scoreTxt, centerMark, spectraSongTime])
						{
			...
			case 'Hunted':
				...
				{
					camHUD.angle = -25;
					camHudMoves = FlxTween.tween(camHUD, {angle: 25}, 3, {ease: FlxEase.sineInOut, type: PINGPONG});
					if (ClientPrefs.flashing)
						camGame.flash(FlxColor.WHITE, 1.5);
					if (ClientPrefs.shaders)
						if (!ClientPrefs.lowQuality)
				...
				{
					camHUD.angle = 0;
					camHudMoves = FlxTween.tween(camHUD, {angle: 0}, 3, {ease: FlxEase.sineInOut, type: PINGPONG});
					camBars.flash(FlxColor.BLACK, 2);
					if (!ClientPrefs.lowQuality)
					{
						if (ClientPrefs.bloom)
