...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		bg.loadGraphic(Paths.image('$path/Title_bg'), false);
		bg.screenCenter();
		bg.scale.x = 0.68;
		bg.scale.y = 0.67;
		add(bg);
	...
	function windowFixesAndEvents()
		{
			if(Application.current.window.title.contains("Funkin.avi: Scrapped - Hi, wanna see me glitch?"))
				{
					new FlxTimer().start(3, function(tmr:FlxTimer)
						{
							Application.current.window.title = "Funkin.avi: Scrapped - I'm starting to glitch now, oooooo";
							new FlxTimer().start(3, function(tmr:FlxTimer)
							{
								Application.current.window.title = "Funkin.avi: Scrapped - That's cool, ain't it?";
								new FlxTimer().start(1, function(tmr:FlxTimer)
								{
									Application.current.window.title = "Funkin.avi: Scrapped - Wait...";
									new FlxTimer().start(1, function(tmr:FlxTimer)
									{
										Application.current.window.title = "Funkin.avi: Scrapped - What's going on here?";
										new FlxTimer().start(1, function(tmr:FlxTimer)
										{
											Application.current.window.title = "Funkin.avi: Scrapped - Why am I still glitching?";
											new FlxTimer().start(1, function(tmr:FlxTimer)
											{
												Application.current.window.title = "Funkin.avi: Scrapped - oh no...";
												new FlxTimer().start(1, function(tmr:FlxTimer)
												{
													Application.current.window.title = "Funkin.avi: Scrapped - oh god, oh fuck, PLAYER, PLEASE HELP ME!";
													new FlxTimer().start(1, function(tmr:FlxTimer)
													{
														Application.current.window.title = "Funkin.avi: Scrapped - I BEG OF YOU";
														new FlxTimer().start(1, function(tmr:FlxTimer)
														{
															Application.current.window.title = "Funkin.avi: Scrapped - JUST GO TO THE MAIN MENU ALREADY, I CAN'T STOP AAAAAAAAAAAAAAAAAAAAAAA";
															new FlxTimer().start(1, function(tmr:FlxTimer)
															{
																Application.current.window.title = "Funkin.avi: Scrapped - AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA";
																new FlxTimer().start(1, function(tmr:FlxTimer)
																{
																	Application.current.window.title = "Funkin.avi: Scrapped - WHAT ARE YOU WAITING FOR??????";
																	new FlxTimer().start(1, function(tmr:FlxTimer)
																	{
																		Application.current.window.title = "Funkin.avi: Scrapped - JUST GO ALREADY, JUST FUCKING PRESS ENTER";
																		new FlxTimer().start(1, function(tmr:FlxTimer)
																		{
																			Application.current.window.title = "Funkin.avi: Scrapped - OH GOD, THE GLITCH IS GETTING WORSE";
																			new FlxTimer().start(1, function(tmr:FlxTimer)
																			{
																				Application.current.window.title = "Funkin.avi: Scrapped - WHY DID I THINK THIS WAS A GOOD IDEA?";
																				new FlxTimer().start(1, function(tmr:FlxTimer)
																				{
																					Application.current.window.title = "Funkin.avi: Scrapped - OH THE MISERY EVERYBODY WANNA BE MY ENEMY MY ENEMY";
																					new FlxTimer().start(1, function(tmr:FlxTimer)
																						{
																							Application.current.window.title = "Funkin.avi: Scrapped - Hi, wanna see me glitch?";
																						});
																				});
																			});
																		});
				...
				else if(Application.current.window.title.contains("Funkin.avi: Scrapped - I'm shutting down your game now, fuck you"))
					{
						new FlxTimer().start(2, function(tmr:FlxTimer){
							System.exit(0);
						});
					}
				else if(Application.current.window.title.contains('Funkin.avi: Scrapped - Fuck you *inverts your game*'))
				{
					if (ClientPrefs.shaders)
					{
				...
				else if(Application.current.window.title.contains('Funkin.avi: Scrapped - fuckin.mp3 - i juss shat meseff'))
					{
						Application.current.window.title = "fuckin.mp3 - i juss shat meseff";
					}
				else if(Application.current.window.title.contains("Funkin.avi: Scrapped -  "))
					{
						Application.current.window.title = " ";
					}
					else if(Application.current.window.title.contains('Funkin.avi: Scrapped - Funny Date Fact...'))
						{
							// this one's special because we gotta prevent spoilers for the newies
							if (GameData.tgLock == 'locked' && GameData.blessLock == 'locked')
								Application.current.window.title = "Funkin.avi: Scrapped - Two of our characters are dating, this is canon.";
							else if (GameData.tgLock == 'locked' && GameData.blessLock == 'beaten')
								Application.current.window.title = "Funkin.avi: Scrapped - A Special Guest & White Noise are dating, this is canon.";
							if (GameData.tgLock == 'beaten' && GameData.blessLock == 'locked')
								Application.current.window.title = "Funkin.avi: Scrapped - Mr. Smiles & A Special Guest are dating, this is canon.";
							else Application.current.window.title = "Funkin.avi: Scrapped - Mr. Smiles & White Noise are dating, this is canon.";
						}
		}
}